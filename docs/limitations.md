...
The expected labels in `examples/data` were authored with the cases for this repository and are not independently reviewed; they exercise the framework. `examples/public-benchmarks` runs against datasets with their own labels: RAGTruth (human span-level), HaluEval (model-generated, filtered) and τ-bench (task reward only). See `examples/public-benchmarks/RESULTS.md` for the recorded numbers and their weak spots.
...
- Judging quality on your data. One recorded RAGTruth run is one dataset, one rubric wording and one day.
