...
- **Bug**: open an issue with the *Bug report* template. Include the failing command, the case id, rubric id and the `answers` block from the run JSON when it concerns judge behaviour. Never paste real customer data or API keys.
- **Feature or new check kind**: open an issue with the *Feature request* template first, with an example case and the outcome you expect. Agreeing on the outcome before the code saves everyone time.
- **Question**: use Discussions, not issues.
- **Security**: use the private advisory form linked in `SECURITY.md`.
- **Pull request**: fork, branch from `main` (`fix/…`, `feat/…`, `docs/…`), keep it focused, fill in the PR template, and make sure CI is green. Title in the imperative ("Add OTel span import", "Fix review count in compare"). A maintainer reviews within a few days; small PRs get merged fastest. Squash-merge is the default, so commit granularity inside the PR does not matter.
