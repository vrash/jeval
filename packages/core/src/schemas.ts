...
export const RubricSchema = z.object({
  ...
  params: z
    .object({
      ...
      /**
       * claim-support: `response` asks one question about the whole response (default);
       * `sentence` asks one question per sentence in the same request and reports which sentences failed.
       */
      granularity: z.enum(["response", "sentence"]).optional(),
      /** claim-support with sentence granularity: cap on sentences judged (the tail is merged). Default 40. */
      maxSentences: z.number().int().min(1).max(120).optional(),
      /**
       * grounded-values: which case fields count as sources for numbers, dates and times in the output.
       * Default: context, toolEvents, input, policy.
       */
      groundIn: z.array(z.enum(["context", "toolEvents", "input", "policy", "messages"])).optional(),
      /**
       * grounded-values: value kinds to check. Default: money, percent, date, time, duration.
       * Bare numbers are opt-in: summaries legitimately derive counts ("two options") that appear in no source.
       */
      valueKinds: z.array(z.enum(["number", "money", "percent", "date", "time", "duration"])).optional(),
      /**
       * Ask the judge twice with independently phrased instructions in the same request and decide on the
       * averaged probabilities; a disagreement between readers forces review. Default 1.
       */
      readers: z.union([z.literal(1), z.literal(2)]).optional(),
...
export const LabelRecordSchema = z.object({
  caseId: idSchema,
  /** A rubric id, or `<rubricId>#<partId>` for a sentence-level part label. */
  rubricId: z
    .string()
    .min(1)
    .max(260)
    .regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*(#[A-Za-z0-9._-]+)?$/, "rubricId may be a rubric id or rubricId#partId"),
