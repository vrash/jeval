...
export interface CheckResult {
  ...
  /**
   * Probability mass behind each side of the decision, for automation curves:
   * `acceptable` is the support for pass, `unacceptable` the support for fail (including
   * uncovered claims when the rubric treats them as failures). Absent for deterministic outcomes.
   */
  scores?: { acceptable: number; unacceptable: number };
  /** Per-part outcomes for checks that judged parts of the output separately (sentence-level claim support). */
  parts?: CheckPart[];
  error?: { kind: string; message: string };
}

export interface CheckPart {
  /** Stable part id (s1, s2, … for sentences). */
  id: string;
  text: string;
  /** Character offsets into the judged output. */
  start: number;
  end: number;
  ...
  reason: string;
  questionKey: string;
  /** Decision scores for this part (see CheckResult.scores). */
  scores?: { acceptable: number; unacceptable: number };
}
