...
function renderParts(check: CheckResult): string {
  if (!check.parts || check.parts.length === 0) return "";
  const rows = check.parts
    .map(
      (p) =>
        `<tr><td>${statusCell(p.status)}</td><td><code>${escapeHtml(p.id)}</code></td><td>${escapeHtml(p.text)}</td><td class="muted">${escapeHtml(p.reason)}</td></tr>`,
    )
    .join("");
  return `<h3>Sentences</h3><table><thead><tr><th>Status</th><th>Id</th><th>Sentence</th><th>Reason</th></tr></thead><tbody>${rows}</tbody></table>`;
}
