            ...
            <details>
              ...
              <div className="mt-2 space-y-2 text-sm">
                ...
                {ch.parts && ch.parts.length > 0 ? (
                  <div>
                    <p className="font-medium">Sentence by sentence</p>
                    <ul className="mt-1 divide-y divide-line rounded-md border border-line">
                      {ch.parts.map((p) => (
                        <li key={p.id} className="flex flex-wrap items-start gap-2 p-2 text-sm">
                          <StatusBadge status={p.status} size="sm" />
                          <span className="font-mono text-xs text-fg-faint">{p.id}</span>
